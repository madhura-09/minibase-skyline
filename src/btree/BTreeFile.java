...
public class BTreeFile extends IndexFile
	...
	private	BTSortedPage currentLastPage;
	private RID currentLastRID = new RID();
...
//	}
	...
	public BTSortedPage getLastLeafPage(BTreeHeaderPage header)
			throws  IOException,
			IteratorException,
			ConstructPageException,
			HashEntryNotFoundException,
			...
			InvalidFrameNumberException
	{
		int keyType = header.get_keyType();
		BTSortedPage sortedPage=new BTSortedPage(header.get_rootId(),keyType);
		while ( sortedPage.getType()==NodeType.INDEX ) {
			BTIndexPage indexPage=new BTIndexPage((Page)sortedPage, keyType);
			sortedPage = new BTSortedPage(indexPage.getLeftLink(),keyType);
			SystemDefs.JavabaseBM.unpinPage(indexPage.getCurPage(), true/*dirty*/);
		}
		while ( sortedPage.getType()==NodeType.LEAF) {
			...
			if(leafPage.getNextPage().pid ==  INVALID_PAGE){
				SystemDefs.JavabaseBM.unpinPage(leafPage.getCurPage(), true/*dirty*/);
				break;
			}
			else{
				sortedPage = new BTSortedPage(leafPage.getNextPage(),keyType);
				SystemDefs.JavabaseBM.unpinPage(leafPage.getCurPage(), true/*dirty*/);
			}
		}
		return sortedPage;
	...
	public RID getNextLastRID() throws IteratorException, IOException, HashEntryNotFoundException, InvalidFrameNumberException, PageUnpinnedException, ReplacerException, ConstructPageException {
		RID dataRID = new RID();
		if(firstCall){
			firstCall=false;
			currentLastPage = getLastLeafPage(getHeaderPage());
			dataRID = ((LeafData) new BTLeafPage((Page)currentLastPage, AttrType.attrReal).getLast().data).getData();
			currentLastRID.pageNo = currentLastPage.firstRecord().pageNo;
			currentLastRID.slotNo = currentLastPage.getSlotCnt()-1;
		}
		else{
			if(currentLastRID.slotNo > 0){
				dataRID = ((LeafData) new BTLeafPage((Page)currentLastPage, AttrType.attrReal).getPrev(currentLastRID).data).getData();

			}
			else{
				currentLastPage = new BTSortedPage (new BTLeafPage((Page)currentLastPage, AttrType.attrReal).getPrevPage(),AttrType.attrReal);
				dataRID = ((LeafData) new BTLeafPage((Page)currentLastPage, AttrType.attrReal).getLast().data).getData();
				currentLastRID.pageNo = currentLastPage.firstRecord().pageNo;
				currentLastRID.slotNo = currentLastPage.getSlotCnt()-1;
			}
