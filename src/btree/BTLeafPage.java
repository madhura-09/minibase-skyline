...
public class BTLeafPage extends BTSortedPage {
  ...
  public KeyDataEntry getFirst(RID rid) 
    ...
    /**  Iterators.
     * One of the two functions: getLast and getPrev
     * which  provide an iterator interface to the records on a BTLeafPage.
     *@param rid It will be modified and the last rid in the leaf page
     * will be passed out by itself. Input and Output parameter.
     *@return return the first KeyDataEntry in the leaf page.
     * null if no more record
     *@exception  IteratorException iterator error
     */
    public KeyDataEntry getLast()
            throws  IteratorException
    {

        KeyDataEntry  entry;

        try { // begin with last slot

            if ( getSlotCnt() < 0) {
                return null;
            }
            entry=BT.getEntryFromBytes(getpage(), getSlotOffset(getSlotCnt()-1), getSlotLength(getSlotCnt()-1),
                    keyType, NodeType.LEAF);
     ...
     throws  IteratorException
   {
    ...
    /**Iterators.
     * One of the two functions: getLast and getPrev which  provide an
     * iterator interface to the records on a BTLeafPage.
     *@param rid It will be modified and the prev rid will be passed out
     *by itself. Input and Output parameter.
     *@return return the prev KeyDataEntry in the leaf page.
     *null if no more record.
     *@exception IteratorException iterator error
     */
