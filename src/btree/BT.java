...
public class BT  implements GlobalConst{
   ...
   */  
  public final static float keyCompare(KeyClass key1, KeyClass key2)
    throws KeyNotMatchException
    {
        if ( (key1 instanceof IntegerKey) && (key2 instanceof IntegerKey) ) {

            return (float) (((IntegerKey)key1).getKey()).intValue()
                    - (((IntegerKey)key2).getKey()).intValue();
        }
        else if ( (key1 instanceof FloatKey) && (key2 instanceof FloatKey) ) {
//            System.out.println("(((FloatKey)key1).getKey()).floatValue():"+(((FloatKey)key1).getKey()).floatValue()
//            +"    (((FloatKey)key2).getKey()).floatValue():"+(((FloatKey)key2).getKey()).floatValue()+"\nFinal: "+(int) Math.ceil((((FloatKey)key1).getKey()).floatValue()
//                    - (((FloatKey)key2).getKey()).floatValue()));
            return  (((FloatKey)key1).getKey()).floatValue()
                                - (((FloatKey)key2).getKey()).floatValue();
        }
      else if  ( (key1 instanceof StringKey) && (key2 instanceof StringKey)){
        return (float) ((StringKey)key1).getKey().compareTo(((StringKey)key2).getKey());
      }
  ...
  protected final static 
	...
	if ( keyType== AttrType.attrInteger) {
	  key= new IntegerKey( new Integer 
              ...
              key= new FloatKey( new Float
                      (Convert.getFloValue(offset, from)));
          }
  ...
  private static void _printAllLeafPages(PageId currentPageId,  int keyType) 
    ...
    public static void getLastLeafPage(BTreeHeaderPage header)
            throws  IOException,
            IteratorException,
            ConstructPageException,
            HashEntryNotFoundException,
            ...
            InvalidFrameNumberException
    {
        ...
        //SystemDefs.JavabaseBM.unpinPage(header.get_rootId(), true/*dirty*/);
        //return (BTLeafPage) sortedPage;
    }
