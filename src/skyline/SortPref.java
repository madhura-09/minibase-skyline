...
public class SortPref extends Iterator implements GlobalConst
{
	...
	*/
            ...
            Exception
    {
        Tuple tuple;
        pnode cur_node;
        pnodeSplayPQPref Q1 = new pnodeSplayPQPref(_sort_pref, order, Q.Attr_type );
        pnodeSplayPQPref Q2 = new pnodeSplayPQPref(_sort_pref, order, Q.Attr_type );
        ...
        while (true) {
            ...
            // System.out.println("CompareTupleWithTuplePref in SortPref");
            comp_res = Methods.CompareTupleWithTuplePref(cur_node.tuple,Q.Attr_type,lastElem, Q.Attr_type,(short) 0,temp, _sort_pref,_sort_pref.length );  // need tuple_utils.java

            // comp_res = Methods.Dominates(cur_node.tuple,Q.Attr_type,lastElem, Q.Attr_type,(short) 0,temp, _sort_pref,_sort_pref.length );  // need tuple_utils.java
            ...
            if ((comp_res < 0 && order.tupleOrder == TupleOrder.Ascending) || (comp_res > 0 && order.tupleOrder == TupleOrder.Descending)) {
                ...
                TupleUtils.SetValue(lastElem, cur_node.tuple, _sort_pref, Q.Attr_type);
...
//        }
        ...
        for (int i = 0; i < n_cols; i++) {
            switch (sortFldType[i].attrType) {
                ...
                case AttrType.attrInteger:
                    ...
                    lastElem.setIntFld(i + 1, Integer.MAX_VALUE);
                    break;
                case AttrType.attrReal:
                    //      lastElem.setHdr(fld-no, junk, null);
                    lastElem.setFloFld(i + 1, Float.MAX_VALUE);
                    break;
                case AttrType.attrString:
                    //      lastElem.setHdr(fld_no, junk, s_size);
                    lastElem.setStrFld(i + 1, s);
     ...
     * @param pref_list the field number of the field to sort on
     * @param sort_order the sorting order (ASCENDING, DESCENDING)
     * @param pref_list_length the length of the sort field
    ...
    throws IOException, SortException
    {
        ...
        pref_len=pref_list_length;

        Q = new pnodeSplayPQPref(_sort_pref, order,_in);
